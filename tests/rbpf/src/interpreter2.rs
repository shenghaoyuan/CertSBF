...
pub struct Interpreter2<'a, 'b, C: ContextObject> {
...
impl<'a, 'b, C: ContextObject> Interpreter2<'a, 'b, C> {
    ...
    pub fn step(&mut self) -> bool {
        ...
        let sbpf_version = self.executable.get_sbpf_version();
                ...
                match insn.opc & 0x08 {
                    ebpf::BPF_K =>{
                        match insn.opc {
                            ...
                            ebpf::SUB32_IMM  => if sbpf_version.swap_sub_reg_imm_operands() {
                            ...
                            ebpf::MUL32_IMM  if !sbpf_version.enable_pqr() => self.reg[dst] = (self.reg[dst] as i32).wrapping_mul(insn.imm as i32)      as u64,
                            ebpf::DIV32_IMM  if !sbpf_version.enable_pqr() => self.reg[dst] = (self.reg[dst] as u32             / insn.imm as u32)      as u64,
                            ...
                            ebpf::NEG32      if sbpf_version.enable_neg() => self.reg[dst] = (self.reg[dst] as i32).wrapping_neg()                     as u64 & (u32::MAX as u64),
                            ebpf::MOD32_IMM  if !sbpf_version.enable_pqr() => self.reg[dst] = (self.reg[dst] as u32             % insn.imm as u32)      as u64,
                            ...
                            ebpf::LE if sbpf_version.enable_le() => {
                    ...
                    ebpf::BPF_X =>{
                        match insn.opc {
                            ...
                            ebpf::MUL32_REG  if !sbpf_version.enable_pqr() => self.reg[dst] = (self.reg[dst] as i32).wrapping_mul(self.reg[src] as i32) as u64,
            ...
            ebpf::BPF_ALU64   => {
                match insn.opc & 0x08 {
                    ebpf::BPF_K =>{
                        match insn.opc {
                            ebpf::ADD64_IMM => {
                                if dst == STACK_PTR_REG && sbpf_version.dynamic_stack_frames() {
                            ...
                            ebpf::SUB64_IMM  => if sbpf_version.swap_sub_reg_imm_operands() {
                                                self.reg[dst] =  (insn.imm as u64).wrapping_sub(self.reg[dst])
                            } else {
                                                self.reg[dst] =  self.reg[dst].wrapping_sub(insn.imm as u64)
                            },
                            ebpf::MUL64_IMM  if !sbpf_version.enable_pqr() => self.reg[dst] =  self.reg[dst].wrapping_mul(insn.imm as u64),
                            ebpf::DIV64_IMM  if !sbpf_version.enable_pqr() => self.reg[dst] /= insn.imm as u64,
                            ...
                            ebpf::NEG64      if sbpf_version.enable_neg() => self.reg[dst] = (self.reg[dst] as i64).wrapping_neg() as u64,
                            ebpf::MOD64_IMM  if !sbpf_version.enable_pqr() => self.reg[dst] %= insn.imm as u64,
                    ...
                    ebpf::BPF_X =>{
                        match insn.opc {
                            ...
                            ebpf::MUL64_REG  if !sbpf_version.enable_pqr() => self.reg[dst] =  self.reg[dst].wrapping_mul(self.reg[src]),
                ...
                match insn.opc & 0x08 {
                    ebpf::BPF_K =>{
                        match insn.opc {
                            ...
                            ebpf::CALL_IMM   => {
                                ...
                                let (external, internal) = if sbpf_version.static_syscalls() {
                    ...
                    ebpf::BPF_X =>{
                        match insn.opc {
                            ...
                            ebpf::CALL_REG   => {
                                let target_pc = if sbpf_version.callx_uses_src_reg() {
                                    self.reg[src]
                                } else {
                                ...
                                if sbpf_version.static_syscalls() && self.executable.get_function_registry().lookup_by_key(next_pc as u32).is_none() {
                                    self.vm.due_insn_count += 1;
                                    self.reg[11] = next_pc;
                                    throw_error!(self, EbpfError::UnsupportedInstruction);
                                }
