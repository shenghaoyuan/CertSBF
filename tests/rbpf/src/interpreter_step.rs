...
macro_rules! throw_error {
    ($self:expr, $err:expr) => {{
        ...
        println!("{:?}",$self.vm.program_result);
        return (false, 0, 0, 0);
    }};
...
pub struct InterpreterStep<'a, 'b, C: ContextObject> {
...
impl<'a, 'b, C: ContextObject> InterpreterStep<'a, 'b, C> {
    ...
    fn push_frame(&mut self, config: &Config) -> (bool, u64, u64, u64) {
    ...
    #[rustfmt::skip]
    pub fn step(&mut self) -> (bool, u64, u64, u64) {
        /*println!("1 :");
        println!("  r{}: 0x{:016X}", self.rx_index, self.reg[self.rx_index as usize]);*/
        ...
        match insn.opc {
                ...
                if external {
                    if let Some((_function_name, function)) = self.executable.get_loader().get_function_registry().lookup_by_key(insn.imm as u32) {
                        ...
                        self.reg[0] = match &self.vm.program_result {
                            ...
                            ProgramResult::Err(_err) => return (false, 0, 0,0 ),
                        };
                    }
                }
            ...
            _ => throw_error!(self, EbpfError::UnsupportedInstructionT),
        }
        ...
        /*println!("2:");
        println!("  r{}: 0x{:016X}", self.rx_index, self.reg[self.rx_index as usize]);
        println!("  pc: 0x{:016X}",  self.reg[11]);*/
        (true, self.rx_index as u64, self.reg[self.rx_index], self.reg[11])
    }
}
