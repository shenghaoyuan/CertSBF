...
use crate::{
    ...
    interpreter_step::InterpreterStep,
...
impl<'a, C: ContextObject> EbpfVm<'a, C> {
    ...
    /// Execute the program in our way 
   pub fn execute_program2(
        &mut self,
        executable: &Executable<C>,
        interpreted: bool,
    ) -> (u64, ProgramResult) {
        ...
        if interpreted {
            #[cfg(feature = "debugger")]
            let debug_port = self.debug_port.clone();
            let mut interpreter = Interpreter2::new(self, executable, self.registers);
            #[cfg(feature = "debugger")]
            if let Some(debug_port) = debug_port {
                crate::debugger::execute(&mut interpreter, debug_port);
            } else {
    ...
    pub fn execute_step(
        ...
        for (reg, val) in register_map {
            if let Some(reg_num) = reg.strip_prefix('r') {
                if let Ok(index) = reg_num.parse::<usize>() {
                    if index < 10 {
        ...
        self.registers[11] = executable.get_entrypoint_instruction_offset() as u64;
        if istore | isload {
            self.registers[0]= ebpf::MM_INPUT_START;
        }
        let mut interpreter = InterpreterStep::new(self, executable, self.registers, rx_index);
