module Interp_test2 : sig
  ...
  type bpf_state
  val bpf_interp_test :
    nat list -> nat list -> nat list -> nat -> nat -> nat -> bool
end = struct
...
let rec bit_int
  x0 n = match x0, n with Neg (Bit1 m), Suc n -> bit_int (Neg (inc m)) n
    ...
    | Pos One, Suc n -> false
    | Neg (Bit1 m), Zero -> true
    | Neg (Bit0 m), Zero -> false
    | Pos (Bit1 m), Zero -> true
    | Pos (Bit0 m), Zero -> false
    | Pos One, Zero -> true
    | Neg One, n -> true
    | Zero_int, n -> false;;
...
let rec power _A a x1 = match a, x1 with a, Zero -> one _A.one_power
...
let rec times_nat x0 n = match x0, n with Zero, n -> Zero
                    | Suc m, n -> plus_nat n (times_nat m n);;

let one_nat : nat = Suc Zero;;
...
type bpf_state =
  BPF_OK of
    num1 bit0 bit0 bit0 bit0 bit0 bit0 word *
      ...
      num1 bit0 bit0 bit0 bit0 bit0 bit0 word *
      num1 bit0 bit0 bit0 bit0 bit0 bit0 word
  | BPF_Sucess of num1 bit0 bit0 bit0 bit0 bit0 bit0 word | BPF_EFlag |
  BPF_Err;;
...
let rec option_val_of_u64
  v = (match v with None -> None | Some v1 -> Some (Vlong v1));;
...
let rec of_nat_aux _A inc x1 i = match inc, x1, i with inc, Zero, i -> i
...
let rec the_signed_int _A
  w = signed_take_bit ring_bit_operations_int
        (minus_nat (len_of _A.len0_len Type) (Suc Zero)) (the_int _A w);;
...
let rec eval_load_imm
  dst imm1 imm2 rs mem =
    (let sv1 = eval_snd_op_i64 (SOImm imm1) rs in
     let sv2 = eval_snd_op_i64 (SOImm imm2) rs in
     let vm_addr =
       cast (len_signed
              (len_bit0
                (len_bit0
         ...
         (plus_word
           ...
           sv1 sv2)
       in
      (match loadv M64 mem vm_addr with None -> None | Some Vundef -> None
        | Some (Vint v) ->
    ...
    (match
      ...
      | Some iv ->
        (let pc1 = (if is_v1 then eval_reg iv rs else eval_reg src rs) in
          (match push_frame rs ss is_v1 pc1 enable_stack_frame_gaps
            with (None, _) -> None
            | (Some ssa, rsa) ->
              (if less_word
                    (len_bit0
...
let rec eval_call_imm
  imm rs ss is_v1 fm enable_stack_frame_gaps =
    (match
      get_function_registry
        (cast (len_signed
                (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1))))))
          (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1))))) imm)
        fm
      with None -> None
      | Some pc ->
        (match push_frame rs ss is_v1 pc enable_stack_frame_gaps
          with (None, _) -> None | (Some ssa, rsa) -> Some (pc, (rsa, ssa))));;
    ...
    (let sv = eval_snd_op_u64 (SOReg sop) rs in
     let vm_addr =
      ...
      (match loadv chk mem vm_addr with None -> None | Some Vundef -> None
        | Some (Vbyte _) -> None | Some (Vshort _) -> None
        | Some (Vint v) ->
          Some (fun_upd equal_bpf_ireg rs dst
                 (cast (len_bit0
                         (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1)))))
...
let rec size_list x = gen_length Zero x;;

let rec equal_nat x0 x1 = match x0, x1 with Zero, Suc x2 -> false
                    | Suc x2, Zero -> false
    ...
    = (let is_v1 = (match sv with V1 -> true | V2 -> false) in
        (match ins
          with BPF_LD_IMM (dst, imm1, imm2) ->
            (match eval_load_imm dst imm1 imm2 rs m with None -> BPF_EFlag
              | Some rsa ->
                BPF_OK
                  (plus_word
                     (len_bit0
          ...
          | BPF_CALL_IMM (_, imm) ->
            (match eval_call_imm imm rs ss is_v1 fm enable_stack_frame_gaps
              with None -> BPF_EFlag
              | Some (pca, (rsa, ssa)) ->
                BPF_OK (pca, rsa, m, ssa, sv, fm, cur_cu, remain_cu))
          | BPF_EXIT ->
                         ...
                         (len_bit0
                           (len_bit0
                             (len_bit0
                               (len_bit0 (len_bit0 (len_bit0 len_num1))))))
                         remain_cu cur_cu
                     then BPF_EFlag else BPF_Sucess (rs BR0))
...
let rec bpf_find_instr
  pc l =
    (let npc = times_nat pc iNSN_SIZE in
     ...
     let dst =
       unsigned_bitfield_extract_u8 Zero (nat_of_num (Bit0 (Bit0 One))) reg
       in
     let src =
...
(len_bit0 (len_bit0 len_num1))))))
 ...
 (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1))))))))
          ...
          | BPF_Sucess a -> BPF_Sucess a | BPF_EFlag -> BPF_EFlag
          | BPF_Err -> BPF_Err);;
  ...
  m n v =
    BPF_OK
      (zero_word
         (len_bit0
           (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1)))))),
        init_reg_map, m, init_stack_state, v, init_func_map,
        zero_word
          (len_bit0
            (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1)))))),
        n);;
...
let rec int_to_u8_list
  lp = map (of_nat (len_bit0 (len_bit0 (len_bit0 len_num1)))) lp;;
      ...
      bpf_interp (plus_nat fuel one_nat) (int_to_u8_list lp)
        (init_bpf_state (u8_list_to_mem (int_to_u8_list lm))
          (of_nat
            (len_bit0
              (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1))))))
            (plus_nat fuel one_nat))
          (if equal_nat v one_nat then V1 else V2))
        true
        (of_int
          (len_bit0
            (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1))))))
