...
let rec nat_of_int (n: int64) : nat =
...
(* 修改后的 num_of_int 函数 *)
let num_of_int (n: int64) : num =
  if n <= 0L then failwith "num_of_int: n must be positive"
  else
    let bits = ref [] in
    let n_ref = ref n in
    (* 构建位列表 *)
    while !n_ref > 0L do
      let bit = if Int64.rem !n_ref 2L = 0L then 0 else 1 in
      bits := bit :: !bits;
      n_ref := Int64.div !n_ref 2L
    done;
    (* 使用位列表构建 num 值 *)
    let num = List.fold_left (fun acc bit ->
      if bit = 0 then Bit0 acc else Bit1 acc
    ) One !bits in
    num

let int_of_standard_int (n: int64) : myint =
  if n = 0L then Zero_int
  else if n > 0L then Pos (num_of_int n)
  else Neg (num_of_int (Int64.neg n))

let int_list_of_standard_int_list (lst: int64 list) : myint list =
...
(len_bit0 (len_bit0 len_num1))))))
  ...
  (len_bit0 (len_bit0 (len_bit0 (len_bit0 (len_bit0 len_num1))))))))
          ...
          | BPF_Success a -> let _ = print_endline ("hello 9") in BPF_Success a | BPF_EFlag -> let _ = print_endline ("hello 3") in BPF_EFlag
