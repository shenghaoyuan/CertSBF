...
proof-
  ...
  thus ?thesis using a3 a4 b4 b5 b6 b8 a1 push_pop_subgoal_rr_aux2_3 a2
    ...
    by (smt (verit, ccfv_threshold) insert_iff)
qed
...
proof-
  ...
  have b7_1:"length xins \<ge> (2::nat)" using a0 by simp
  have b7_2:"Next reg2 m2 = interp3 (butlast xins) (Next reg m)" using b3 b0 a0 by (simp add: b6)
  have b7:"Next reg'' m'' = (exec_instr (last xins) 1 reg2 m2)" using interp3_length4_aux4 b2_0 b7_1 b7_2 interp3_list_aux3 a1 interp3_list_aux3 a1 outcome.inject
    by (metis interp3_length4_aux6)
  have b8:"Next reg'' m'' = (exec_instr (last xins) 1 reg2 m')" using b7 b4 by simp
  thus ?thesis using a3 a4 b4 b5 b6 b8 a1 push_pop_subgoal_rr_aux2_3 a2
    using a0 b3 butlast.simps(2) list.distinct(1) list.sel(3)
...
proof-
  ...
  have b7_1:"length xins \<ge> (2::nat)" using a0 by simp
  have b7_2:"Next reg2 m2 = interp3 (butlast xins) (Next reg m)" using b3 b0 a0 by (simp add: b6)
  have b7:"Next reg'' m'' = (exec_instr (last xins) 1 reg2 m2)" using interp3_length4_aux4 interp3_list_aux3 b2_0 b7_1 b7_2 interp3_list_aux3 a1 outcome.inject
    by (metis interp3_length4_aux6)
  have b8:"Next reg'' m'' = (exec_instr (last xins) 1 reg2 m2)" using b7 b4 by simp
  have b9:"x64Syntax.RCX \<in> {x64Syntax.RDX, x64Syntax.RAX, x64Syntax.RCX}" by simp
  thus ?thesis using a3 a4 a1 push_pop_subgoal_rr_aux2_3 a2 b3 b2_1 b7 b4
    by (smt (verit, best) a0 b5 butlast.simps(2) list.sel(3) list.simps(3))
qed
...
lemma shift_subgoal_rr_aux4:
  assumes a0:"xins = [Ppushl_r x64Syntax.RCX, Pmovq_rr (x64Syntax.RCX) (bpf_to_x64_reg src) ,op (bpf_to_x64_reg dst),Ppopl x64Syntax.RCX] " and
    a1:"Next reg'' m'' = interp3 xins (Next reg m) " and
    a2: "op \<in> {Pshrq_r, Pshlq_r, Psarq_r}" and
    a3:"Vlong n1 = reg (IR SP)"
  shows" \<forall> r \<noteq> dst. reg'' (IR  (bpf_to_x64_reg r)) = reg (IR (bpf_to_x64_reg r))" 
proof-
  have b0:"reg'' (IR x64Syntax.RCX) = reg (IR x64Syntax.RCX)" using a0 a1 a2 shl_subgoal_rr_aux4_1 shr_subgoal_rr_aux4_1 sar_subgoal_rr_aux4_1 a3 by auto
...
proof -
  ...
  moreover have b0:"xins = [Ppushl_r x64Syntax.RCX, Pmovq_rr (x64Syntax.RCX)(bpf_to_x64_reg src),Pshlq_r (bpf_to_x64_reg dst),Ppopl x64Syntax.RCX]" 
    ...
    have b4:"\<forall> r \<noteq> dst. reg'(IR (bpf_to_x64_reg r)) = reg (IR (bpf_to_x64_reg r))" using b0 a5 shift_subgoal_rr_aux4 a9 by simp
