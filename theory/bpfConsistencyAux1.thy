...
proof-
  ...
  then obtain reg' m' where b0:"Next reg' m' = (exec_instr (xins!0) 1 reg m)" by blast
  ...
  then obtain reg'' m'' where b1:" Next reg'' m'' = (exec_instr (xins!1) 1 reg' m')"
    by auto
  have b4:"Next reg'' m'' = Next reg2 m2" using  a1 interp3_list_aux3 b1 exec_instr_def
    by (metis (no_types, lifting) a0 b0 interp3.simps(2) list_consists_2 numeral_1_eq_Suc_0 numeral_eq_one_iff outcome.simps(4))
...
lemma push_pop_subgoal_rr_aux1:
  assumes a0:"hd xins = Ppushl_r tmpreg" and 
          ...
          a2:"tmpreg \<in> {x64Syntax.RDX, x64Syntax.RAX, x64Syntax.RCX}" and
          a3:"Vlong addr = sub64 (reg (IR SP)) (vlong_of_memory_chunk M32)"
        shows "result = Next reg' m' \<longrightarrow> storev M32 m addr (reg (IR tmpreg)) \<noteq> None"
proof (rule ccontr)
  assume a2:"\<not> (result = Next reg' m' \<longrightarrow> storev M32 m addr (reg (IR tmpreg)) \<noteq> None)"
  let ?tmp = "storev M32 m addr (reg (IR tmpreg)) "
  let ?res_ok = "Next reg' m'"
...
lemma push_pop_subgoal_rr_aux2_1_1:" Vlong addr = sub64 (reg (IR SP)) (vlong_of_memory_chunk M32) \<Longrightarrow> 
    storev M32 m addr (reg (IR tmpreg)) = Some m' \<Longrightarrow>
    xins = Ppushl_r tmpreg \<Longrightarrow> 
    Next reg' m' = (exec_instr xins 1 reg m) \<Longrightarrow> 
    reg' (IR SP) = sub64 (reg (IR SP)) (vlong_of_memory_chunk M32)"
...
lemma push_pop_subgoal_rr_aux2_1:
  assumes a0:"xins = [Ppushl_r tmpreg, Ppopl tmpreg]" and 
    ...
    a3:"tmpreg \<in> {x64Syntax.RDX, x64Syntax.RAX, x64Syntax.RCX}" and
    a4:"Vlong addr = sub64 (reg (IR SP)) (vlong_of_memory_chunk M32)"
...
proof -
  ...
  let "?v" = " (reg (IR tmpreg))"
  have c4:"reg''(IR tmpreg) = ?v" using exec_pop_def c2 push_pop_subgoal_rr_aux2_1_2 a2 a3
    using One_nat_def a0 b3 nth_Cons_0 nth_Cons_Suc by (metis a4)
...
proof-
  ...
  have b1:"\<exists> n2. Vlong n2 = sub64 (reg (IR SP)) (vlong_of_memory_chunk M32)" using a3 sub64_def
    by (metis (no_types, lifting) memory_chunk.simps(15) val.simps(29) vlong_of_memory_chunk_def)
  thus ?thesis using b0 a0 a1 push_pop_subgoal_rr_aux2_1 interp3_length2_aux2 a2 a3
    by metis
qed
