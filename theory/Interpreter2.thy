...
imports
  ...
  ebpf rBPFDecoder Interpreter Interpreter1
begin
...
definition step2 :: "u64 \<Rightarrow> bpf_instruction \<Rightarrow> reg_map \<Rightarrow> mem \<Rightarrow> stack_state \<Rightarrow> SBPFV \<Rightarrow> u64 \<Rightarrow> u64 \<Rightarrow> bpf_state1" where
"step2 pc ins rs m ss sv cur_cu remain_cu = ( let is_v1 = (case sv of V1 \<Rightarrow> True | _ \<Rightarrow> False) in
  ...
  BPF_ALU bop d sop \<Rightarrow> (
    ...
    OKS rs' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs' m ss sv cur_cu remain_cu ) |
  ...
  BPF_ADD_STK i \<Rightarrow> (
    ...
    Some ss' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs m ss' sv cur_cu remain_cu ) |
  ...
  BPF_LDX chk dst sop off \<Rightarrow> (
    ...
    Some rs' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs' m ss sv cur_cu remain_cu ) |

  BPF_ST chk dst sop off \<Rightarrow> (
    case eval_store chk dst sop off rs m of
    None \<Rightarrow> bad_bpf_state1 BPF_EFlag1 |
    Some m' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs m' ss sv cur_cu remain_cu ) |
  ...
  BPF_JA off  \<Rightarrow>
    BPF_st BPF_OK1 (pc + scast off + 1) rs m ss sv cur_cu remain_cu  |
  ...
  BPF_JUMP cond bpf_ireg snd_op off  \<Rightarrow> (
    if eval_jmp cond bpf_ireg snd_op rs then
      ...
      BPF_st BPF_OK1 (pc + 1) rs m ss sv cur_cu remain_cu' ) |
  ...
  BPF_CALL_IMM src imm \<Rightarrow> (
    ...
    BPF_st BPF_OK1 pc rs' m ss' sv cur_cu remain_cu' ) |
