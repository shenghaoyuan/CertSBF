...
lemma aluq_subgoal_rr_aux3:"op2 \<in> {BPF_SUB, BPF_MOV, BPF_ADD, BPF_AND, BPF_XOR, BPF_OR} \<Longrightarrow>
    bins = BPF_ALU64 op2 dst (SOReg src) \<Longrightarrow> (BPF_OK pc rs' m' ss' is_v1 fm (cur_cu+1) remain_cu) = step fuel bins rs m ss is_v1 fm enable_stack_frame_gaps program_vm_addr cur_cu remain_cu \<Longrightarrow>
