...
datatype bpf_state1 =
  BPF_st bpf_state_ty u64 u64 reg_map mem stack_state SBPFV u64 
                                                     
fun default_bpf_state1 :: "bpf_state_ty => bpf_state1" where
"default_bpf_state1 ty = (
  BPF_st ty 0 0 (\<lambda> _. 0) (\<lambda> _. None) default_stack_state V1 0 
)"
...
"step1 pc ins rs m ss sv cur_cu remain_cu = ( let is_v1 = (case sv of V1 \<Rightarrow> True | _ \<Rightarrow> False) in
  ...
  BPF_ALU bop d sop \<Rightarrow> (
    ...
    OKS rs' \<Rightarrow> BPF_st BPF_OK1 (cur_cu+1) (pc+1) rs' m ss sv remain_cu ) |
  ...
  BPF_ADD_STK i \<Rightarrow> (
    ...
    Some ss' \<Rightarrow> BPF_st BPF_OK1 (cur_cu+1) (pc+1) rs m ss' sv remain_cu ) |
  ...
  BPF_LDX chk dst sop off \<Rightarrow> (
    ...
    Some rs' \<Rightarrow> BPF_st BPF_OK1 (cur_cu+1) (pc+1) rs' m ss sv remain_cu ) |

  BPF_ST chk dst sop off \<Rightarrow> (
    case eval_store chk dst sop off rs m of
    None \<Rightarrow> default_bpf_state1 BPF_EFlag1 |
    Some m' \<Rightarrow> BPF_st BPF_OK1 (cur_cu+1) (pc+1) rs m' ss sv remain_cu ) |
  ...
  BPF_JA off  \<Rightarrow>
    BPF_st BPF_OK1 (cur_cu+1) (pc + scast off + 1) rs m ss sv remain_cu  |

  BPF_JUMP cond bpf_ireg snd_op off  \<Rightarrow> (
    if eval_jmp cond bpf_ireg snd_op rs  then
      BPF_st BPF_OK1 (cur_cu+1) (pc + scast off + 1) rs m ss sv remain_cu 
    else
      BPF_st BPF_OK1 (cur_cu+1) (pc + 1) rs m ss sv remain_cu ) |

  BPF_CALL_IMM src imm \<Rightarrow> (
    case eval_call_imm imm rs ss is_v1  of
    None \<Rightarrow> default_bpf_state1 BPF_EFlag1 |
    Some (pc, rs', ss') \<Rightarrow> BPF_st BPF_OK1 (cur_cu+1) pc rs' m ss' sv remain_cu ) |
  ...
  BPF_EXIT \<Rightarrow> (
    if call_depth ss = 0 then
      if cur_cu >  remain_cu then
        default_bpf_state1 BPF_EFlag1
      else
        BPF_st BPF_Success1 cur_cu pc rs m ss sv remain_cu
    else (
      let (pc', rs', ss') = eval_exit rs ss is_v1 in
        BPF_st BPF_OK1 (cur_cu+1) pc' rs' m ss' sv remain_cu ))
)"

fun bpf_interp1 :: "nat \<Rightarrow> bpf_bin \<Rightarrow> bpf_state1 \<Rightarrow> bpf_state1" where
"bpf_interp1 0 _ _ = default_bpf_state1 BPF_EFlag1 " | 
"bpf_interp1 (Suc n) prog st  = (
  case st of
  BPF_st ty lpc pc rs m ss sv remain_cu \<Rightarrow> (
    case ty of
    BPF_OK1 \<Rightarrow> (
    if INSN_SIZE*unat pc < length prog then
      if lpc \<ge> remain_cu then
        default_bpf_state1 BPF_CU1
      else
        case bpf_find_instr (unat pc) prog of
        None \<Rightarrow> default_bpf_state1 BPF_Err1 |
        Some ins \<Rightarrow> bpf_interp1 n prog (step1 pc ins rs m ss sv lpc remain_cu) 
    else default_bpf_state1 BPF_Err1) |
    _ \<Rightarrow> st )
)"
...
"refine_bpf_state1 st = (
  ...
  BPF_st ty lpc pc rs m ss sv remain_cu \<Rightarrow> (
    case ty of
    BPF_OK1 \<Rightarrow> BPF_OK pc rs m ss sv lpc remain_cu |
...
" st1 = bpf_interp1 n l st0 \<Longrightarrow>
  ...
  subgoal for k l st0 st1 st
    apply simp
    apply (cases st0)
    subgoal for x1 x2 x3 x4 x5 x6 x7 x8 
      apply (simp add: refine_bpf_state1_def)
      apply (cases x1; simp)
      apply (cases "INSN_SIZE * unat x3 < length l"; simp)
      apply (cases "x8 \<le> x2"; simp)
      apply (cases "bpf_find_instr (unat x3) l"; simp)
      subgoal for a
        apply (subgoal_tac "step x3 a x4 x5 x6 x7 x2 x8 = (refine_bpf_state1 (step1 x3 a x4 x5 x6 x7 x2 x8))")
         prefer 2
        subgoal
        ...
        apply simp
        apply (cases "step1 x3 a x4 x5 x6 x7 x2 x8"; simp add: refine_bpf_state1_def)
        done
      done
    done
  done
