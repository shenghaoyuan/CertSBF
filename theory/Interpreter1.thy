...
datatype bpf_state1 =
  BPF_st bpf_state_ty  u64 reg_map mem stack_state SBPFV u64 u64
                                                     
fun bad_bpf_state1 :: "bpf_state_ty => bpf_state1" where
"bad_bpf_state1 ty = (
  BPF_st ty 0 (\<lambda> _. 0) (\<lambda> _. None) default_stack_state V1 0 0
)"
...
"step1 pc ins rs m ss sv cur_cu remain_cu = ( let is_v1 = (case sv of V1 \<Rightarrow> True | _ \<Rightarrow> False) in
  ...
  BPF_ALU bop d sop \<Rightarrow> (
    ...
    OKS rs' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs' m ss sv (cur_cu+1) remain_cu ) |
  ...
  BPF_ADD_STK i \<Rightarrow> (
    ...
    Some ss' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs m ss' sv (cur_cu+1) remain_cu ) |
  ...
  BPF_LDX chk dst sop off \<Rightarrow> (
    ...
    Some rs' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs' m ss sv (cur_cu+1) remain_cu ) |

  BPF_ST chk dst sop off \<Rightarrow> (
    case eval_store chk dst sop off rs m of
    None \<Rightarrow> bad_bpf_state1 BPF_EFlag1 |
    Some m' \<Rightarrow> BPF_st BPF_OK1 (pc+1) rs m' ss sv (cur_cu+1) remain_cu ) |
  ...
  BPF_JA off  \<Rightarrow>
    BPF_st BPF_OK1 (pc + scast off + 1) rs m ss sv (cur_cu+1) remain_cu  |

  BPF_JUMP cond bpf_ireg snd_op off  \<Rightarrow> (
    if eval_jmp cond bpf_ireg snd_op rs  then
      BPF_st BPF_OK1 (pc + scast off + 1) rs m ss sv (cur_cu+1) remain_cu 
    else
      BPF_st BPF_OK1 (pc + 1) rs m ss sv (cur_cu+1) remain_cu ) |

  BPF_CALL_IMM src imm \<Rightarrow> (
    case eval_call_imm imm rs ss is_v1  of
    None \<Rightarrow> bad_bpf_state1 BPF_EFlag1 |
    Some (pc, rs', ss') \<Rightarrow> BPF_st BPF_OK1 pc rs' m ss' sv (cur_cu+1) remain_cu ) |
  ...
  BPF_EXIT \<Rightarrow> (
    if call_depth ss = 0 then
      if cur_cu >  remain_cu then
        ...
        BPF_st BPF_Success1 pc rs m ss sv cur_cu remain_cu
    else (
      let (pc', rs', ss') = eval_exit rs ss is_v1 in
        BPF_st BPF_OK1 pc' rs' m ss' sv (cur_cu+1) remain_cu ))
)"
...
" st1 = bpf_interp1 n l st0 \<Longrightarrow>
  ...
  subgoal for k l st0 st1 st
    ...
    subgoal for x1 x2 x3 x4 x5 x6 x7 x8
      ...
      apply (cases x1; simp)
      apply (cases "INSN_SIZE * unat x2 < length l"; simp)
      apply (cases "x8 \<le> x7"; simp)
      apply (cases "bpf_find_instr (unat x2) l"; simp)
      subgoal for a
        apply (subgoal_tac "step x2 a x3 x4 x5 x6 x7 x8 = (refine_bpf_state1 (step1 x2 a x3 x4 x5 x6 x7 x8))")
         prefer 2
        subgoal
        ...
        apply simp
        apply (cases "step1 x2 a x3 x4 x5 x6 x7 x8"; simp add: refine_bpf_state1_def)
        done
      done
    done
  done
