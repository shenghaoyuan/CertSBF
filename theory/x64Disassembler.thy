...
"x64_decode pc l_bin = (
  let h = l_bin!pc in
    ...
    \<comment> \<open> R7 legacy \<close>
      ...
      let h1 = l_bin!(pc+1) in
        if bitfield_extract_u8 4 4 h1 \<noteq> 0b0100 then  
          \<comment> \<open> R7.1 [legacy + opcode + modrm + imm] \<close>
          let reg = l_bin!(pc+2) in
          let modrm = bitfield_extract_u8 6 2 reg in
          let reg1  = bitfield_extract_u8 3 3 reg in
          let reg2  = bitfield_extract_u8 0 3 reg in
          ...
          let w = bitfield_extract_u8 3 1 h1 in
          let r = bitfield_extract_u8 2 1 h1 in
          let x = bitfield_extract_u8 1 1 h1 in
          let b = bitfield_extract_u8 0 1 h1 in
            ...
            let dst   = bitfield_insert_u8 3 1 reg2 b in
              ...
              \<comment> \<open> P2882 ` MOV: reg to memory`  ->  `66H 0100 0RXB : 1000 1001 : mod reg r/m `\<close>
                if modrm = 0b01  then
                  let (imm::u32) = scast (l_bin!(pc+4)) in  \<comment> \<open> displacement8 \<close>
                      case ireg_of_u8 src of None \<Rightarrow> None | Some src \<Rightarrow> (
                      case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> ( 
                        if w = 0 \<and> x = 0 then
    ...
    let op = l_bin!(pc+1) in
      ...
      else if bitfield_extract_u8 3 5 op = 0b11001 then
        let reg2 = bitfield_extract_u8 0 3 op in
        let dst  = bitfield_insert_u8 3 1 reg2 0 in
          case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> (
            Some (2, Pbswapl dst))
      ...
      else if bitfield_extract_u8 4 4 op = 0b0100 then
        let flag = (bitfield_extract_u8 0 4 op) in 
        let reg = l_bin!(pc+2) in 
        let modrm = bitfield_extract_u8 6 2 reg in
        let reg1  = bitfield_extract_u8 3 3 reg in
        let reg2  = bitfield_extract_u8 0 3 reg in
      ...
      else if bitfield_extract_u8 4 4 op = 0b1000 then
        let flag = (bitfield_extract_u8 0 4 op) in
  ...
  else if bitfield_extract_u8 4 4 h \<noteq> 0b0100 then  \<comment> \<open> h is not rex  \<close>
      \<comment> \<open> R1 [opcode] \<close>
      \<comment> \<open> P2885 `PUSH: qwordregister (alternate encoding)`   -> ` 0100 W00BS : 0101 0 reg64` \<close>
      if bitfield_extract_u8 3 5 h = 0b01010 then
        let reg2 = bitfield_extract_u8 0 3 h in
        let dst  = bitfield_insert_u8 3 1 reg2 0 in
        case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> (
          Some (1, Ppushl_r dst))
      \<comment> \<open> P2885 `POP: qwordregister (alternate encoding)`   -> ` 0100 W00B : 0101 1 reg64 ` \<close>
      else if bitfield_extract_u8 3 5 h = 0b01011 then
        let reg2 = bitfield_extract_u8 0 3 h in
        let dst  = bitfield_insert_u8 3 1 reg2 0 in
        case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> (
          Some (1, Ppopl dst))
    ...
    else if bitfield_extract_u8 0 4 h = 0 then   \<comment> \<open> h is rex, the low 4-bit must not 0  \<close> 
      None
    else  \<comment> \<open> h is rex  \<close> 
      let w = bitfield_extract_u8 3 1 h in
      let r = bitfield_extract_u8 2 1 h in
      let x = bitfield_extract_u8 1 1 h in
      let b = bitfield_extract_u8 0 1 h in
      let op = l_bin!(pc+1) in 
      \<comment> \<open> R6.1 [rex + opcode] \<close>
      if op = 0x99 then
        Some (2, Pcqo)
      \<comment> \<open> P2885 `PUSH: qwordregister (alternate encoding)`   -> ` 0100 W00BS : 0101 0 reg64` \<close>
      else if bitfield_extract_u8 3 5 op = 0b01010 then
        let reg2 = bitfield_extract_u8 0 3 op in
        let dst  = bitfield_insert_u8 3 1 reg2 b in
        case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> (
          if w = 0 \<and> r = 0 \<and> x = 0 then
      ...
      \<comment> \<open> P2885 `POP: qwordregister (alternate encoding)`   -> ` 0100 W00B : 0101 1 reg64 ` \<close>
      else if bitfield_extract_u8 3 5 op = 0b01011 then
        let reg2 = bitfield_extract_u8 0 3 op in
        let dst  = bitfield_insert_u8 3 1 reg2 b in
        case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> (
          if w = 0 \<and> r = 0 \<and> x = 0 then
      ...
      else if bitfield_extract_u8 3 5 op = 0b10111 then
        let reg2 = bitfield_extract_u8 0 3 op in
        let dst  = bitfield_insert_u8 3 1 reg2 b in
        let i1 = l_bin!(pc+2)  in
        let i2 = l_bin!(pc+3)  in
      ...
      if bitfield_extract_u8 3 5 op1 = 0b11001 then
        let reg2 = bitfield_extract_u8 0 3 op1 in
        let dst  = bitfield_insert_u8 3 1 reg2 b in
          case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> (
      ...
      \<comment> \<open> P2919 `MOVcc : qwordregister2 to qwordregister1` -> ` 0100 1R0B 0000 1111: 0100 tttn : 11 qwordreg1 qwordreg2` \<close>
        else if bitfield_extract_u8 4 4 op1 = 0b0100 then
          let flag = (bitfield_extract_u8 0 4 op1)  in 
          ...
          if op = 0x89 then
            ...
            \<comment> \<open> P2882 `MOV qwordregister1 to qwordregister2` -> `0100 1R0B : 1000 1001 : 11 reg1 reg2` \<close>
              ...
              let (dis::u32) = scast (l_bin!(pc+3)) in  \<comment> \<open> displacement8 \<close>
                ...
                case ireg_of_u8 dst of None \<Rightarrow> None | Some dst \<Rightarrow> ( 
                  if w = 1 \<and> x = 0 then
                    Some (4, Pmov_mr (Addrmode (Some dst) None dis) src M64)
                  else if w = 0 \<and> x = 0 then
              ...
              if reg2 \<noteq> 0b100 then 
                ...
                let rbase  = bitfield_extract_u8 0 3 sib in
                let rindex = bitfield_extract_u8 3 3 sib in
                let scale  = bitfield_extract_u8 6 2 sib in
...
value "bitfield_extract_u8 2 1 64" (**r r = 0 *)
value "bitfield_extract_u8 0 1 64" (**r b = 0 *)
value "bitfield_extract_u8 6 2 195" (**r modrm = 3 *)
value "bitfield_extract_u8 3 3 200" (**r reg1 = 0 *)
value "bitfield_extract_u8 0 3 195" (**r reg2 = 3 *)
